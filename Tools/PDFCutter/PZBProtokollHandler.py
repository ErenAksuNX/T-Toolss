...
class PZBDocHandler:
    def __init__(self):
        ...
        self.PZBIdentifier = ['ebi', 'cab', 'pzb']

    def IsThisaPZBProtokoll(self, AllTokens):

        # Checks if Page is a PZB document

        AllTokens = self.Vali.lowercastTokens(AllTokens)
        if len(set(self.PZBIdentifier) - set(AllTokens)) == 0:
            return True
        return False

    def getPZBDokumentName(self, AllTokens):
        ...
        if Fahrzeugnummer is None:
            Fahrzeugnummer = self.Vali.getZugNummer(AllTokens)
        if Date is not None and Fahrzeugnummer is not None:
            ReleaseDocName = Fahrzeugnummer + '_' + str(Date.year) + str(Date.month) + str(Date.day) + '_' + 'PZBs'
        else:
            ReleaseDocName = 'PZBDokument'
        return ReleaseDocName

    def isDefinedName(self, PageName):

        # Checks if the Dokument ist a 'PZBDOkument' which is specified.
