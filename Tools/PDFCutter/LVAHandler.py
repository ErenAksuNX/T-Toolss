...
class LVADocHandler():
    ...
    def __init__(self):
        ...
        self.LVAIdentifier = ['betriebsdaten lva', 'lva']

    def IsThisaLVAProtokoll(self, AllTokens):
        # Checks if Page is a PZB document

        AllTokens = self.Vali.lowercastTokens(AllTokens)
        if len(set(self.LVAIdentifier) - set(AllTokens)) == 0:
            return True
        return False

    def getLVADokumentName(self, AllTokens):
        ...
        if Date is not None and Fahrzeugnummer is not None:
            ReleaseDocName = Fahrzeugnummer + '_' + str(Date.year) + str(Date.month) + str(Date.day) + '_' + 'LVA'
        else:
            ReleaseDocName = 'LVADokument'
