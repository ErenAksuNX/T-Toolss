...
class TurHandler:
    ...
    def __init__(self):
        ...
        self.TurIdentifier = ['tür-schließkraftmessung', 'türtyp', 'tür-nr', ]

    def IsThisaTurProtokoll(self, AllTokens):
        # Checks if Page is a PZB document

        AllTokens = self.Vali.lowercastTokens(AllTokens)
        if len(set(self.TurIdentifier) - set(AllTokens)) == 0:
            return True
        return False

    def getTurDokumentName(self, AllTokens):
        ...
        if Date is not None and Fahrzeugnummer is not None:
            ReleaseDocName = Fahrzeugnummer + '_' + str(Date.year) + str(Date.month) + str(Date.day) + '_' + 'Tür'
        else:
            ReleaseDocName = 'TürDokument'
